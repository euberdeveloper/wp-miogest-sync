                    ...
                    <div class="modal-body">
                        ...
                        <form class="mt-4">
                            ...
                            <div class="form-group">
                                ...
                                <span id="pxp-modal-contact-agent-hidden-message" class="d-none"><?php 
									if( ICL_LANGUAGE_CODE =='it'){
										echo sprintf(__('Salve, %s%svorrei informazioni riguardo la proprietà %s codice %s.', 'resideo'), PHP_EOL, PHP_EOL, esc_html($modal_info['title']), get_post_meta($id = get_the_ID(), 'codice', true));
									}else if(ICL_LANGUAGE_CODE=='en'){
										echo sprintf(__('Hi, %s%sI would like information about the property %s code %s.', 'resideo'), PHP_EOL, PHP_EOL, esc_html($modal_info['title']), get_post_meta($id = get_the_ID(), 'codice', true));
									}else if(ICL_LANGUAGE_CODE=='de'){
										echo sprintf(__('Grüße, %s%sIch möchte Informationen zum Objekt %s code %s.', 'resideo'), PHP_EOL, PHP_EOL, esc_html($modal_info['title']), get_post_meta($id = get_the_ID(), 'codice', true));}?></span>
                                <textarea id="pxp-contact-agent-message" rows="4" class="form-control"><?php 
									if( ICL_LANGUAGE_CODE =='it'){
										echo sprintf(__('Salve, %s%svorrei informazioni riguardo la proprietà %s codice %s.', 'resideo'), PHP_EOL, PHP_EOL, esc_html($modal_info['title']), get_post_meta($id = get_the_ID(), 'codice', true));
									}else if(ICL_LANGUAGE_CODE=='en'){
										echo sprintf(__('Hi, %s%sI would like information about the property %s code %s.', 'resideo'), PHP_EOL, PHP_EOL, esc_html($modal_info['title']), get_post_meta($id = get_the_ID(), 'codice', true));
									}else if(ICL_LANGUAGE_CODE=='de'){
										echo sprintf(__('Grüße, %s%sIch möchte Informationen zum Objekt %s code %s.', 'resideo'), PHP_EOL, PHP_EOL, esc_html($modal_info['title']), get_post_meta($id = get_the_ID(), 'codice', true));
									}?></textarea>
                            </div>
