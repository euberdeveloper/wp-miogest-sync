var tinymce, tinyMCE, shortcodeModal;
...
(function($) {
    ...
    shortcodeModal = {
        init: function() {
            var self = this, parentWin, settings;
            ...
            self.content = 
                '<div tabindex="0" role="dialog" id="shortcode-modal" style="position: relative; display: none;">' + 
                                ...
                                '<div class="media-frame-toolbar">' + 
                                    '<div class="media-toolbar">' + 
                                        '<div class="media-toolbar-primary search-form">' + 
                                            '<button type="button" id="cancel-button" class="button media-button button-default button-large">Cancel</button>' + 
                                            '<button type="button" id="insert-button" class="button media-button button-primary button-large">Insert</button>' + 
                                        '</div>' + 
                                    '</div>' + 
                                '</div>' + 
                            '</div>' + 
        ...
        // Open the shortcode window
        open: function(options) {
            var self = this;
            var fields = '';

            $('#shortcode-modal .media-frame-title h1').html(options.title);

            fields += 
                '<form><div class="row">';

            $.each(options.body, function(i, op) {
                switch(op.type) {
                    case 'textbox':
                        fields += 
                    '<div class="col-xs-12 col-sm-12 col-md-6"><div class="form-group">' + 
                        '<label for="' + op.name + '">' + op.label + '</label>' + 
                        '<input type="text" id="' + op.name + '" name="' + op.name + '" class="form-control" value="' + op.value + '" placeholder="' + op.placeholder + '">' + 
                    '</div></div>';
                        break;
                    ...
                    '<div class="col-xs-12 col-sm-12 col-md-6"><div class="form-group">' + 
                        '<label for="' + op.name + '">' + op.label + '</label>' + 
                        '<textarea id="' + op.name + '" name="' + op.name + '" class="form-control" placeholder="' + op.placeholder + '">' + op.value + '</textarea>' + 
                    '</div></div>';
                        break;
                    ...
                    '<div class="col-xs-12 col-sm-12 col-md-6"><div class="form-group">' + 
                        '<label for="' + op.name + '">' + op.label + '</label>' + 
                        '<select id="' + op.name + '" name="' + op.name + '" class="form-control">';
                        $.each(op.values, function(i, v) {
                            fields += 
                    ...
                    '<div class="col-xs-12 col-sm-12 col-md-6"><div class="form-group">' + 
                        '<label for="' + op.name + '">' + 
                            '<input type="checkbox" id="' + op.name + '" name="' + op.name + '" value="1" ';
                        if (op.value == '1') {
                            fields += 
                                'checked="checked"';
                        }
                    ...
                    '<div class="col-xs-12 col-sm-12 col-md-6"><div class="form-group">' + 
                        '<label for="' + op.name + '">' + op.label + '</label>' + 
                        '<input type="text" id="' + op.name + '" name="' + op.name + '" class="color-field" value="' + op.value + '">' + 
                    '</div></div>';
                        break;
                    ...
                    '<div class="col-xs-12 col-sm-12 col-md-6"><div class="form-group">' + 
                        ...
                        '<input type="hidden" id="' + op.name + '" name="' + op.name + '" value="' + op.value + '">' + 
                        '<button type="button" class="button media-button button-default button-large icon-button">'; 
                        if (typeof op.value != 'undefined' && op.value.length) {
                            fields += 
                            '<span class="' + op.value + '"></span> ' + op.value;
                        } else {
                            fields += 
                            '<span class="fa fa-fonticons"></span> fa fa-fonticons';
                        }
                        fields += 
                        '</button>' + 
                    '</div></div>';
                    ...
                    '<div class="col-xs-12 col-sm-12 col-md-6"><div class="form-group">' + 
                        '<label for="' + op.name + '">' + op.label + '</label>' + 
                        '<input type="text" id="' + op.name + '" name="' + op.name + '" class="form-control" value="' + op.value + '">' + 
                        '<button type="button" class="button media-button button-default button-large file-button">Browse...</button>' + 
                    '</div></div>';
                        break;
                }
            });
            ...
            $('#shortcode-modal .media-frame-content').html(fields);

            $('.color-field').wpColorPicker();

            $('#shortcode-modal #insert-button').on('click', function(e) {
                var serFields = $('#shortcode-modal form').serializeArray();
                options.onsubmit(serFields);
                $('#shortcode-modal').hide().remove();
                e.preventDefault();
            });

            $('.icon-button').on('click', function(e) {
                self.openIconsWindow(this);
                e.preventDefault();
            });
        ...
        openIconsWindow: function(btn) {
            ...
            for (var i = 0; i < faIcons.length; i++) {
                faList += 
                    '<div class="col-xs-6 col-sm-4 col-md-3 is-icon">' + 
